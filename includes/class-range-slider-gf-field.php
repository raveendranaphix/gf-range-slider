...
class Range_Slider_GF_Field extends GF_Field {
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'Range Slider', 'range-slider' );
	}
	...
	public function get_form_editor_inline_script_on_page_render() {
		...
		// Set the default field label for the range_slider type field
		$script = sprintf( "function SetDefaultValues_range_slider(field) {field.label = '%s';}", $this->get_form_editor_field_title() ) . PHP_EOL;
		...
		// Saving the range_slider setting
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$input = "<input data-rangeslider name='input_{$id}' id='{$field_id}' type='range' min='0' data-orientation='horizontal' max='100' step='1' value='{$value}' class='{$class}' {$tabindex} {$logic_event} {$placeholder_attribute} {$required_attribute} {$invalid_attribute} {$disabled_text}/><output></output>";

		return sprintf( "<div class='ginput_container ginput_container_%s'>%s</div>", $this->type, $input );
	}
}

GF_Fields::register( new Range_Slider_GF_Field() );
